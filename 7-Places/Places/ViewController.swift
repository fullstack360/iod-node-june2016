...
class ViewController: UIViewController, CLLocationManagerDelegate, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    func showMapView(){
        print("showMapView")
        self.segueToMap(self.venueList)
    }
    
    func segueToMap(venueList: Array<Venue>){
        let mapView = MapViewController()
        mapView.currentLocation = self.currentLocation
        mapView.venueList = venueList
...
//        self.foursquareRequest(latLng, query: searchText)
        self.googlePlacesRequest(latLng, query: searchText)
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath){
        let place = self.venueList[indexPath.row]
        print("didSelectRowAtIndexPath: \(place.name)")
        
        self.segueToMap([place])
    }
...
//            let latLng = "\(currentLocation.coordinate.latitude),\(currentLocation.coordinate.longitude)"
            ...
            print("didUpdateLocations: \(self.currentLocation)")
        }
    }
    
    func googlePlacesRequest(latLng: String, query: String){
        let url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=\(latLng)&radius=500&types=\(query)&key=AIzaSyAeser88W3M1xByLny2F3RJCu8cMuLViUs"
        
//        print("googlePlacesRequest: \(url)")
        Alamofire.request(.GET, url, parameters: nil).responseJSON { response in
            if let json = response.result.value as? Dictionary<String, AnyObject> {
                
                if let results = json["results"] as? Array<Dictionary<String, AnyObject>> {
//                    print("googlePlacesRequest: \(results)")
                    
                    for placeInfo in results {
                        let placeData = Venue()
                        placeData.populate(placeInfo)
                        self.venueList.append(placeData)
                    }
